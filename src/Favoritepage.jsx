...
const Favoritepage = () => {
    ...
    const fetchNotes = async () => {
        try {
            ...
            const { data } = await axios.get('https://notesapp-be-lphc.onrender.com/content/favorites', {
    ...
    const handleDelete = async (noteId) => {
        try {
            const token = localStorage.getItem("token");
            await axios.delete(`https://notesapp-be-lphc.onrender.com/content/favorites/${noteId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setNotes((prevNotes) => {
                prevNotes.forEach(note => {
                  if (!note) console.warn('Null or undefined note found:', note);
                });
                return prevNotes.filter((note) => note && note._id !== noteId);
              });              
        } catch (err) {
    ...
    return (
        <div className="flex flex-row gap-5 min-h-screen p-5">
            ...
            <div className="w-3/4 p-4 relative">
                ...
                <div className="mb-4 flex flex-nowrap gap-5 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
                    {loading ? (
                                    ...
                                    <div key={note._id} className="border border-gray-300 p-5 rounded w-60 mt-10 h-80 flex-shrink-0">
                                        ...
                                        <div className='flex flex-row justify-end mt-1'>
                                            <button onClick={() => handleCopy(note.text)}>
                                                <img src="https://raw.githubusercontent.com/Sangavi002/NotesApp_img/refs/heads/main/copy.png" alt="copy" width="12%" className='ml-36'/>
                                            </button>
                                            <button onClick={() => handleDelete(note._id)}>
                                                <img src="https://raw.githubusercontent.com/Sangavi002/NotesApp_img/refs/heads/main/delete.png" alt="delete" width="75%" className='mr-2'/>
                                            </button>
                                        </div>
                                    </div>
                                );
